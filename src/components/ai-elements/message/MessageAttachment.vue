...
const mediaType = computed(() => (props.data.mediaType?.startsWith('image/') && props.data.url ? 'image' : 'file'))
const isImage = computed(() => mediaType.value === 'image')
const attachmentLabel = computed(() => filename.value || (isImage.value ? 'Image' : 'Attachment'))
</script>

<template>
  <div :class="cn('group relative size-24 overflow-hidden rounded-lg', props.class)" v-bind="$attrs">
    <template v-if="isImage">
      <img :src="props.data.url" :alt="filename || 'attachment'" class="size-full object-cover" height="100" width="100" />
      <Button
