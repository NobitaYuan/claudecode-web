...
const label = computed(() => {
  ...
  // }
  const lablesForZHCN: Record<ExtendedToolState, string> = {
    'input-streaming': '等待中（Pending）',
    'input-available': '运行中（Running）',
    'approval-requested': '等待批准（Awaiting Approval）',
    'approval-responded': '已批准（Responded）',
    'output-available': '已完成（Completed）',
    'output-error': '错误（Error）',
    'output-denied': '已拒绝（Denied）',
  }
  return lablesForZHCN[props.state]
})
