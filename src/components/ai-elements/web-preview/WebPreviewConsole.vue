...
<template>
  <Collapsible :class="cn('border-t bg-muted/50 font-mono text-sm', props.class)" :open="isConsoleOpen" v-bind="$attrs" @update:open="context.setConsoleOpen">
    <CollapsibleTrigger as-child>
      <Button class="flex w-full items-center justify-between p-4 text-left font-medium hover:bg-muted/50" type="button" variant="ghost">
        Console
        <ChevronDownIcon :class="cn('h-4 w-4 transition-transform duration-200', isConsoleOpen ? 'rotate-180' : 'rotate-0')" />
      </Button>
    </CollapsibleTrigger>
    <CollapsibleContent
    ...
    >
      <div class="max-h-48 space-y-1 overflow-y-auto">
        ...
        <template v-else>
          <div v-for="(log, index) in props.logs" :key="`${log.timestamp.getTime?.() ?? index}-${index}`" :class="cn('text-xs', levelClass(log.level))">
